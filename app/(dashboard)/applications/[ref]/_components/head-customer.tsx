...
function HeadCustomer({customerRef, status}: Props) {
    ...
    return (
        <header className={'flex gap-4 items-center'}>
            <h2 className={'text-3xl font-semibold text-gray-900 '}>Ref - {customerRef}</h2>
            {status === "pending" && <>
                ...
                ${riskLevel > 75? "bg-green-500" 
                    : (riskLevel > 45 && riskLevel < 75) ?
                        "bg-yellow-500" : 
                        "bg-red-500"}`}>
                    <p className={'text-lg font-semibold'}>{riskLevel}% {riskLevel > 75 ? "Low Risk"
                        : (riskLevel > 45 && riskLevel < 75) ?
                            "Medium Risk" :
                            "High Risk"}</p>
                </div>
            ...
            {status === "approved" && <> <div className={'rounded-lg bg-green-500 px-5 py-1.5 text-white flex gap-2'}>
                    <UserCheck/>
                    <span>
                        Accept
                    </span>
