...
function Page() {
    ...
    return (
        <div className={'items-center h-dvh grid grid-rows-[auto,1fr,auto] gap-6 p-4 w-full'}>
            ...
            <ScrollArea className={"w-full flex gap-2 flex-col items-center justify-center overflow-auto h-full"}>
                <p className={"text-sm text-gray-500 text-center mb-4"}>You will sign a document acknowledging and confirming the followings sets of information.</p>

                {terms.map(term => <TermsItem key={term} text={term} onCheck={(checkedState) => {
                    console.log(checkedState)
                    if (checkedState) setCheckedBoxes(state => [...state, true])
                    else setCheckedBoxes(state => state.filter((_, index) => index !== checkedBoxes.length - 1))
                }}/>)}

            </ScrollArea>

            <div className={"flex items-center justify-between w-full"}>
                <Link href={"/mobile/nationality"}>
                    <Button variant={"ghost"} size={"lg"}> Back </Button>
                </Link>
                <Link href={"/mobile/tasks"}>
                    <Button size={'lg'} disabled={!(checkedBoxes.length >= terms.length)}>I Confirm</Button>
                </Link>
            </div>
        </div>
    );
